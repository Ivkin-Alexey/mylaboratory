...
import { DEFAULT_SEARCH_TERM, BASE_API_URL, encodeQueryParams } from '@/constants';
...
export function useExternalEquipmentList(searchTerm: string = DEFAULT_SEARCH_TERM) {
  ...
  const url = `${BASE_API_URL}/api/equipments${encodedParams}`;
...
export function useExternalEquipmentById(equipmentId: string) {
  const url = `${BASE_API_URL}/api/equipment/${equipmentId}?login=user1`;
...
export function useFavoriteEquipment() {
  ...
  const toggleFavorite = async (equipmentId: number, isFavorite: boolean | undefined) => {
    setIsLoading(true);
    try {
      const url = `${BASE_API_URL}/api/equipments/favorite`;
